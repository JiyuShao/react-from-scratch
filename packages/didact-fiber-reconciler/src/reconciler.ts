...
declare let requestIdleCallback: (fn: IdleRequestCallback) => number;
...
/**
 * 执行渲染/更新工作
 * @param {IIdleDeadline} deadline requestIdleCallback 传来，用于检测空闲时间
 */
function performWork(deadline: IIdleDeadline) {
  workLoop(deadline);
 ...
 * @param {IIdleDeadline} deadline requestIdleCallback() 的参数
 */
function workLoop(deadline: IIdleDeadline) {
